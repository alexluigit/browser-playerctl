class YoutubeMusic extends MprisBase {
  ...
  getPosition(callback) {
    ...
    if (!$("ytmusic-player-bar .time-info").length) return callback(0);
  ...
  playPause(callback) {
    ...
    if ($('[title="Pause"]').length) $('[title="Pause"]').click();

    if ($('[title="Play"]').length) $('[title="Play"]').click();
  ...
  update(callback) {
    if ($('[title="Play"]').length) {
      ...
      else this.media.PlaybackStatus = "Paused";
    } else if ($('[title="Pause"]').length) {
      this.media.PlaybackStatus = "Playing";
    }

    if (
      $('[aria-label="Next song"]').length &&
      !$('[aria-label="Next song"]').prop("disabled")
    )
      this.media.CanGoNext = true;
    else this.media.CanGoNext = false;

    if (
      $('[aria-label="Previous song"]').length &&
      !$('[aria-label="Previous song"]').prop("disabled")
    )
      this.media.CanGoPrevious = true;
    else this.media.CanGoPrevious = false;

    if ($("ytmusic-player-bar .content-info-wrapper .title").length) {
      this.media.Metadata["mpris:trackid"] = $(
        "ytmusic-player-bar .content-info-wrapper .title"
      ).text();
      this.media.Metadata["xesam:title"] = $(
        "ytmusic-player-bar .content-info-wrapper .title"
      ).text();
    }

    if ($("ytmusic-player-bar .image").length)
      this.media.Metadata["mpris:artUrl"] = $("ytmusic-player-bar .image").attr(
        "src"
      );
    ...
    if (
      $("ytmusic-player-bar .content-info-wrapper .subtitle").length &&
      $("ytmusic-player-bar .content-info-wrapper .subtitle").text() &&
      $("ytmusic-player-bar .content-info-wrapper .subtitle").text().split("•")
        .length > 2
    ) {
      this.media.Metadata["xesam:album"] = $(
        "ytmusic-player-bar .content-info-wrapper .subtitle"
      )
        .text()
        .split("•")[1]
        .trim();
      this.media.Metadata["xesam:artist"] = [
        $("ytmusic-player-bar .content-info-wrapper .subtitle")
          .text()
          .split("•")[0]
          .trim(),
      ];
      this.media.Metadata["xesam:albumArtist"] = [
        $("ytmusic-player-bar .content-info-wrapper .subtitle")
          .text()
          .split("•")[0]
          .trim(),
    ...
    if ($("ytmusic-player-bar .time-info").length) {
      ...
      if (timestamp.length == 2) {
        this.media.Metadata["mpris:length"] = this.microSeconds(
          timestamp[1].trim()
        );
      }
    }
